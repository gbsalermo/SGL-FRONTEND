...
**O SGL está funcionalmente aprovado. As Etapas 1 e 2 da pré-produção foram concluídas. A Etapa 3 foi implementada em `feat/etapa-3-residuos`: 3.1 está validada e 3.2/3.3 aguardam validação integrada. A Gestão já pode abrir a prévia do rótulo antes da liberação, enquanto a impressão continua bloqueada até `LIBERADO_PARA_ARMAZENAMENTO`. Se os testes passarem, fechar a Etapa 3 e iniciar a Etapa 4. Não tratar a matriz de permissões como tarefa imediata até o encerramento desse bloco. Conferir `main`, router e Swagger antes de confiar em documentos históricos, e lembrar que alterações funcionais de backend serão implementadas manualmente pelo responsável do projeto.**
