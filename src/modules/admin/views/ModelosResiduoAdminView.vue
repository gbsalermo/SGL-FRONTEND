...
function validar() {
  ...
  if (form.value.riscos.length === 0) throw new Error('Selecione pelo menos um risco.')
  if (form.value.classesIds.length === 0) throw new Error('Selecione pelo menos uma classe de resíduo.')
  if (form.value.medidasSeguranca.length === 0) throw new Error('Selecione pelo menos uma medida de Segurança / EPI.')
    ...
    <div v-if="editorAberto" class="backdrop" @click.self="fecharEditor">
      <section class="editor" role="dialog" aria-modal="true" aria-label="Cadastro de modelo de resíduo">
        ...
        <div class="editor-body">
          ...
          <label><span>Descrição</span><textarea v-model="form.descricao" rows="2" maxlength="1000" required /></label>
          <label><span>Processo de origem</span><textarea v-model="form.processoOrigem" rows="2" maxlength="1000" required /></label>

          <div class="grid three">
            <label><span>Recipiente</span><input v-model="form.recipiente" maxlength="255" required /></label>
          ...
          <fieldset class="model-choice-group">
...
body.sgl-dark-active .models-page .notice {
  border-color: #315071;
  background: #111f33;
  color: #b8c6d8;
}

body.sgl-dark-active .models-page .feedback.success {
  border-color: #2d674d;
  background: #163629;
  color: #72dba8;
}

body.sgl-dark-active .models-page .feedback.error {
  border-color: #d94a5a;
  background: #251821;
  color: #ff6677;
}

body.sgl-dark-active .models-page .workspace,
body.sgl-dark-active .models-page .model-card,
body.sgl-dark-active .models-page .editor {
  border-color: var(--sgl-border);
  background: var(--sgl-surface);
}

body.sgl-dark-active .models-page .toolbar,
body.sgl-dark-active .models-page .editor > header,
body.sgl-dark-active .models-page .editor > footer {
  border-color: var(--sgl-border);
  background: var(--sgl-surface-soft);
}

body.sgl-dark-active .models-page :where(
...
body.sgl-dark-active .models-page fieldset,
body.sgl-dark-active .models-page .components article {
  border-color: var(--sgl-border);
  background: var(--sgl-surface-elevated);
}

body.sgl-dark-active .models-page .ghost,
body.sgl-dark-active .models-page .component-actions button {
  border-color: var(--sgl-border-strong);
  background: var(--sgl-surface-elevated);
  color: var(--sgl-text);
}

body.sgl-dark-active .models-page .danger {
  border-color: #8e3b49;
  background: #251821;
  color: #ff6677;
}
