...
const itensAtencao = computed<ItemAtencao[]>(() => {
  ...
  residuosPendentesAnalise.value.forEach((residuo) => {
    itens.push({
      ...
      rota: `/residuos?status=${encodeURIComponent(residuo.status)}&residuo=${encodeURIComponent(residuo.id)}`,
    })
  })
...
.dashboard-refresh:hover:not(:disabled) {
  ...
  border-color: #7e9bc4;
  box-shadow: 0 6px 16px rgb(13 43 94 / 10%);
}
...
.kpi-card--orange,
.kpi-card--amber { border-color: #f1ddbb; }

.dashboard-content-grid {
  min-width: 0;
  display: grid;
  grid-template-columns: minmax(0, 1fr) minmax(0, 1fr) minmax(330px, .98fr);
  gap: 14px;
  align-items: start;
}
.dashboard-panel {
  ...
  background: var(--sgl-surface);
  overflow: hidden;
  box-shadow: 0 4px 14px rgb(17 26 47 / 3%);
}
...
.attention-item::before {
  ...
  background: var(--attention-accent, #e6a523);
  transition: width 180ms ease;
}
.attention-item:hover {
  z-index: 1;
  transform: translateY(-1px) translateX(2px);
  border-color: var(--attention-accent, #9fb6d8);
  box-shadow: 0 8px 18px rgb(13 43 94 / 12%);
}
.attention-item:hover::before { width: 5px; }
.attention-item--pedido:hover { background: #fff7f7; }
.attention-item--estoque:hover,
.attention-item--lote:hover { background: #fffbf3; }
...
.timeline-item {
  ...
  min-height: 58px;
  display: grid;
  grid-template-columns: 40px 18px minmax(0, 1fr) auto 14px;
  align-items: stretch;
  gap: 7px;
  padding: 0 11px;
  border: 1px solid transparent;
...
.timeline-item:hover {
  z-index: 1;
  transform: translateY(-1px) translateX(2px);
  border-color: var(--timeline-accent, #9fb6d8);
  box-shadow: 0 8px 18px rgb(13 43 94 / 11%);
}
...
.summary-tabs button.summary-tab--active {
  border-color: #d7e1ef;
  background: #fff;
  color: #1a4da1;
  box-shadow: 0 3px 10px rgb(13 43 94 / 7%);
}
.summary-tabs svg {
  width: 16px;
  height: 16px;
...
.lab-card {
  ...
  border-radius: 11px;
  background: #fff;
  box-shadow: 0 6px 18px rgb(13 43 94 / 5%);
}
...
.lab-navigation button:hover:not(:disabled) {
  transform: translateY(-1px);
  border-color: #9fb4d2;
  box-shadow: 0 5px 12px rgb(13 43 94 / 9%);
}
.lab-navigation button:disabled { opacity: .4; cursor: default; }
.lab-navigation button svg {
  width: 17px;
  height: 17px;
  fill: none;
  stroke: currentColor;
...
@media (max-width: 1280px) {
  ...
  .dashboard-panel--summary { grid-column: 1 / -1; }
  .dashboard-panel--summary.dashboard-panel--fixed { height: 390px; max-height: 390px; }
  .lab-summary-body { padding-inline: min(8vw, 72px); }
}
...
@media (max-width: 900px) {
  ...
  .dashboard-panel--summary { grid-column: auto; }
  .dashboard-panel--fixed,
  .dashboard-panel--summary.dashboard-panel--fixed {
    height: auto;
    max-height: none;
  }
  .panel-scroll { max-height: 350px; }
  .summary-view { min-height: 330px; }
  .lab-summary-body { padding-inline: 14px; }
}
...
@media (max-width: 700px) {
  ...
  .summary-tabs button { font-size: 8.5px; }
}
