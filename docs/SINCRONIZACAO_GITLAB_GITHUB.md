# Sincronização GitLab ↔ GitHub — SGL Frontend

**Atualizado em:** 22/09/2026  
**Estado:** infraestrutura ativa e validada  
**Repositório local:** frontend `sgl-web`; este documento também registra a arquitetura conjunta com o backend  
...
## 13. Observação específica do frontend

O frontend não define contratos HTTP por conta própria. Depois de sincronizar branches, conferir o Swagger/OpenAPI do backend antes de adaptar services/types. A branch antiga `feat/etapa-4-residuos` do frontend possui a UI histórica da Etapa 4 e deve ser portada seletivamente para `collab/etapa-4-residuos-reconcile` somente depois de fechar os contratos reconciliados no backend.
