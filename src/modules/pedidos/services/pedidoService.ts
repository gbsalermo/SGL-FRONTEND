...
function rotuloEnum(valor: string | null | undefined) {
  ...
  const rotulos: Record<string, string> = {
    BAIXO: 'Baixo', MEDIO: 'Médio', ALTO: 'Alto', INFLAMAVEL: 'Inflamável', RADIOATIVO: 'Radioativo',
    TOXICO: 'Tóxico', CORROSIVO: 'Corrosivo', BIOLOGICO: 'Biológico', VALIDADE: 'Validade',
    MICROBIANO: 'Microbiano', QUIMICO: 'Químico',
  }
...
function loteUtilizavel(lote: LotePedidoResponse) {
  if (!lote.ativo || lote.quantidadeDisponivel <= 0) return false
...
export const pedidoService = {
  ...
  async listarLotesPorEstoque(estoqueId: string) {
    ...
    return data.filter(loteUtilizavel)
  },
