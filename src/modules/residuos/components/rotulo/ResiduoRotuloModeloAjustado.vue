...
const composicao = computed(() => props.dados.componentes
  .map((componente) => componente.concentracaoOuQuantidade?.trim()
    ? `${componente.nomeComponente} — ${componente.concentracaoOuQuantidade}`
    : componente.nomeComponente)
  .join('; '))

function rotuloNivelRisco(nivel: NivelRiscoResiduo) {
  return ({ NENHUM: 'Nenhum', BAIXO: 'Baixo', MEDIO: 'Médio', ALTO: 'Alto' } as Record<NivelRiscoResiduo, string>)[nivel]
}
      ...
      <div class="label-body">
        <aside class="hazards">
          ...
          <div v-if="riscosVisiveis.length" class="hazard-grid">
            <div v-for="risco in riscosVisiveis" :key="risco" class="hazard-item">
              <img
                v-if="caminhoPictogramaResiduo(risco) && !pictogramaFalhou(risco)"
                :src="caminhoPictogramaResiduo(risco) ?? undefined"
        ...
        <section class="information">
          <div class="risk-heading">
            ...
            <h3>{{ rotuloNivelRisco(dados.nivelRisco) }}</h3>
            <p>{{ riscosVisiveis.length ? riscosVisiveis.map(rotuloRiscoResiduo).join(' · ') : 'Nenhum risco específico confirmado.' }}</p>
          </div>
          ...
          <div class="info-grid">
            <div><h4>Processo de origem</h4><p>{{ dados.processoOrigem }}</p></div>
            <div><h4>Recipiente</h4><p>{{ dados.recipiente }}</p></div>
            <div><h4>Armazenamento temporário</h4><p>{{ dados.localArmazenamentoTemporario || 'Não informado' }}</p></div>
            <div><h4>Destino previsto</h4><p>{{ dados.destinoFinalPrevisto || 'Não informado' }}</p></div>
          </div>
...
.identification p { margin: 0 0 1.5mm; font-size: 8pt; font-weight: 800; letter-spacing: .08em; }
.identification h2 { margin: 0; font-size: 19pt; line-height: 1.05; }
.identification > strong { display: block; margin-top: 2mm; font-size: 10pt; }
.origin { display: flex; flex-direction: column; gap: 1mm; font-size: 8pt; line-height: 1.25; }
.origin > strong { font-size: 9pt; }
.label-body { min-height: 0; display: grid; grid-template-columns: 44mm minmax(0, 1fr); gap: 6mm; padding: 5mm 0; }
.hazards { min-height: 0; padding-right: 5mm; border-right: 1px solid #cbd5e1; }
.hazards > p { margin: 0 0 3mm; font-size: 8pt; font-weight: 800; }
.hazard-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 2.5mm; }
.hazard-item { min-width: 0; text-align: center; }
.hazard-item img, .hazard-fallback { width: 17mm; height: 17mm; margin: 0 auto; object-fit: contain; }
.hazard-fallback { display: grid; place-items: center; border: 2.5px solid #dc2626; font-size: 17pt; font-weight: 900; }
.hazard-item small { display: block; margin-top: .7mm; font-size: 6.2pt; line-height: 1.05; }
.no-hazard { padding: 5mm 2mm; border: 1px dashed #94a3b8; font-size: 8pt; text-align: center; }
.information { min-width: 0; min-height: 0; display: grid; grid-template-rows: auto auto auto auto; align-content: start; gap: 3mm; }
.risk-heading span, .composition h4, .info-grid h4, .description span, .quantity span { font-size: 7pt; font-weight: 800; letter-spacing: .04em; text-transform: uppercase; }
.risk-heading h3 { margin: .5mm 0 1mm; font-size: 16pt; line-height: 1; }
.risk-heading p, .composition p, .info-grid p, .dispatch-date { margin: 0; font-size: 8pt; line-height: 1.25; overflow-wrap: anywhere; }
.composition h4, .info-grid h4 { margin: 0 0 1mm; }
.composition p { max-height: 8mm; overflow: hidden; }
.info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 3mm 5mm; }
.info-grid p { max-height: 8mm; overflow: hidden; }
.dispatch-date { margin-top: 0; }
.label-footer { display: grid; grid-template-columns: 38mm minmax(0, 1fr) 35mm; align-items: center; gap: 5mm; min-height: 15mm; padding-top: 4mm; border-top: 1px solid #cbd5e1; }
.embrapa-brand { min-height: 13mm; display: flex; align-items: center; }
.embrapa-brand img { display: block; width: 34mm; max-height: 12mm; object-fit: contain; object-position: left center; }
...
.quantity strong { display: block; margin-top: 1mm; font-size: 15pt; white-space: nowrap; }
@media print { .residuo-label { print-color-adjust: exact; -webkit-print-color-adjust: exact; } }
</style>
